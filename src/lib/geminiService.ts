...
export async function generateCareerGuidance(
  ...
  if (!genAI) {
    console.warn('[PRAGYA Gemini Service] Gemini API key not found. Using simulated career guidance.');
    return generateMockGuidance(message, courses);
  }

  const systemInstruction = AGENT_REGISTRY.career?.systemPrompt || `You are a professional and friendly Career AI Advisor for a student planner application called PRAGYA.
...
- If the student provides courses they are currently studying, try to connect your suggestions to how those courses help or how they can leverage them.`;
  ...
  for (const modelName of models) {
    try {
      console.log(`[PRAGYA Gemini Service] Trying model ${modelName} for career guidance...`);
      ...
      console.warn(`[PRAGYA Gemini Service] Failed with model ${modelName}:`, error);
      lastError = error;
    }
  }

  console.warn('[PRAGYA Gemini Service] All API model attempts failed. Falling back to simulated career guidance. Error was:', lastError);
  return generateMockGuidance(message, courses);
}
