...
export async function analyzeIntent(prompt: string): Promise<IntentAnalysis> {
  ...
  return generateJSON<IntentAnalysis>(
    `You are an expert AI intent classifier for an AI Foundational Assessment Engine called PRAGYA.

Analyze this user prompt: "${prompt}"

Classify the intent and return ONLY this JSON:
{
  "intent": "<one of: assess_classroom | assess_student | reading_intervention | numeracy_intervention | generate_quiz>",
  "domain": "<specific topic, e.g. 'Class 5 Reading', 'Grade 3 Numeracy', 'Subtraction with borrowing'>",
