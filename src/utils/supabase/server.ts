  ...
  async then(resolve: any, reject: any) {
    try {
      ...
      if (result && result.error && (result.error.code === '42P01' || result.error.message?.includes('does not exist') || result.error.message?.includes('violates row-level security'))) {
        console.warn(`[PRAGYA Safe Client] Table/Access issue on "${this.table}" with live database. Falling back to local mock DB.`);
        const fallbackResult = await this.mockBuilder;
        return resolve(fallbackResult);
      }
      return resolve(result);
    } catch (realErr: any) {
      console.warn(`[PRAGYA Safe Client] Real query exceptions on "${this.table}". falling back to local mock DB. Error:`, realErr.message || realErr);
      try {
        const fallbackResult = await this.mockBuilder;
        return resolve(fallbackResult);
      } catch (mockErr) {
...
// Check if credentials are mock/missing. Only allows mock locally.
  ...
  return {
    auth: {
      ...
      async signUp(options: any) {
        ...
        const cookieStore = await cookies();
        cookieStore.set('pragya_mock_session', token, cookieOpts);
        if (response) {
          response.cookies.set('pragya_mock_session', token, cookieOpts);
        }
      ...
      async signOut() {
        ...
        const cookieStore = await cookies();
        const sessionCookie = cookieStore.get('pragya_mock_session');
        ...
        cookieStore.delete('pragya_mock_session');
        if (response) {
          response.cookies.delete('pragya_mock_session');
        }
        return { error: null };
      },
      async exchangeCodeForSession(code: string) {
        ...
        if (!user) {
          const userId = generateUUID();
          user = { id: userId, email: 'dev@pragya.ai', user_metadata: { display_name: 'Developer' } };
          db.users.push(user);
          db.profiles.push({ 
            id: userId, 
            email: user.email, 
      ...
      cookies: {
        ...
        setAll(cookiesToSet) {
          try {
            ...
            console.error('[Supabase createClient] Error setting cookies in setAll:', err);
      ...
      auth: {
        admin: {
          ...
          async getUserById(id: string) {
            ...
            return { data: { user: user || { id, email: 'mock-user@pragya.ai' } }, error: null };
          },
