...
import { Award, Flame, Sparkles, BookOpen, Calculator, ArrowRight, CheckCircle2, Lock } from 'lucide-react';
...
export default function StudentHome({ learner = MOCK_ACTIVE_STUDENT }: StudentHomeProps) {
  const readingStages = getReadingWorldStages(learner);
  const numeracyStages = getNumeracyWorldStages(learner);

  const currentReadingStage = readingStages.find(s => s.status === 'current') || readingStages[3];
  const currentNumeracyStage = numeracyStages.find(s => s.status === 'current') || numeracyStages[2];

  // XP calculation
  const nextLevelXp = 500;
  const xpPercent = Math.min(100, Math.round((learner.xp / nextLevelXp) * 100));
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-amber-100 border-2 border-amber-300 flex items-center justify-center text-3xl md:text-4xl shadow-inner">
              {learner.avatar}
            </div>
            <div>
          ...
          {/* XP Progress Badge */}
          <div className="bg-white/80 backdrop-blur p-4 rounded-2xl border border-amber-200 shadow-sm w-full md:w-64">
            ...
            <p className="text-[11px] text-gray-500 mt-1.5 text-right font-medium">
              Only {nextLevelXp - learner.xp} XP to unlock Level 4!
            </p>
          </div>
        </div>
      </div>
