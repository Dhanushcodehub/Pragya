...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'getUser') {
      ...
      response.cookies.set('pragya_mock_session', token, {
        httpOnly: true,
      ...
      const sessionCookie = await cookieStore.get('pragya_mock_session');
    ...
    console.error('[PRAGYA Mock API] Route handler error:', error);
